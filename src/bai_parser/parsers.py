...
class BaseSingleParser(BaseParser):
    ...
    def _parse_field_from_config(self, field_config, raw_value):
        ...
        # Handle integer parsing for total fields - Fixed the bug here
    ...
    def _parse_fields_from_config(self, values, fields_config):
        ...
        for field_config in fields_config:
            ...
            if index < len(values):
                ...
                # Handle missing fields by setting them to None
                if isinstance(field_config, str):
...
class TransactionDetailParser(BaseSingleParser):
    ...
    def _parse_fields(self, record):
        # fields at the start
        rest = record.fields[:]  # Create a copy to avoid modifying original
        ...
        # fields at the end - handle variable number of fields
        if len(rest) >= 2:
